...
# Make sure the volume is within the limits
clamp() {
  if [ "$1" -lt 0 ]; then
    echo "0"
...
while true; do
  ...
  -i | --increase)
    ...
    if pactl get-sink-mute "$SINK" | grep -q "yes"; then TOGGLEMUTE=1; fi
    shift
    ;;
  -d | --decrease)
    CHANGE=$((-${2:-$DEFAULT_STEP}))
...
if ((CHANGE != 0)); then
  ...
  pactl set-sink-volume "$SINK" "$(clamp "$VOLUME")%"
  if ((VOLUME > 0)) && pactl get-sink-mute "$SINK" | grep -q "yes"; then TOGGLEMUTE=1; fi
fi

# Mute when volume is 0
if ((VOLUME == 0)) && pactl get-sink-mute "$SINK" | grep -q "no"; then TOGGLEMUTE=1; fi
